...
namespace FFF {
	TileGrid::TileGrid(const char* path, u32 tilesWide, u32 tilesTall)
	: Texture(path), tilesWide(tilesWide), tilesTall(tilesTall), widthFraction(), heightFraction() {}

	TileGrid::TileGrid(const char* path, u32 tilesWide)
	: Texture(path), tilesWide(tilesWide), tilesTall(1), widthFraction(), heightFraction() {}
